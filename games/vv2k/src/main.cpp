...
namespace
{
    ...
    void exec_dialogue(int x) {
        ...
        switch (x) {
            ...
            case 3: {
                Concepts::line lc[5] = {
                    {true, true, 0, "BG: 2"},
                    {true, true, 0, "                                                                                                   ENOKI: Let's go for the room-"},
                    {true, true, 0, "                                                                                                   ENOKI: with the big bookshelf!"},
    ...
    void dungeon(int world = 0) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (local_walls_p < w_size && loc > -1) {
                    local_walls[local_walls_p].set(x * 32, y * 32, bn::sprite_items::environment_stone, loc - 1);
        ...
        bn::sprite_animate_action enoki_anim = bn::create_sprite_animate_action_forever(e_builder.release_build(), 8, enoki.tiles_item(), 0, 1, 0, 2);
        ...
        bn::sprite_animate_action maple_anim = bn::create_sprite_animate_action_forever(builder.release_build(), 8, maple.tiles_item(), 0, 1, 0, 2);
        ...
        // A header
        bn::sprite_ptr a_notif = bn::sprite_items::a_button_2.create_sprite(maple_ptr.x(), maple_ptr.y() - 28);
        ...
        while(true) {
            ...
            if (!done) {
                switch(dir) {
                    case 0:
                        maple_anim = bn::create_sprite_animate_action_forever(builder.release_build(), 8, maple.tiles_item(), 1, 0, 2, 0);
                        break;
                    case 1:
                        maple_anim = bn::create_sprite_animate_action_forever(builder.release_build(), 8, maple.tiles_item(), 4, 3, 5, 3);
                        break;
                    case 2:
                        maple_anim = bn::create_sprite_animate_action_forever(builder.release_build(), 8, maple.tiles_item(), 7, 6, 8, 6);
                        break;
                    case 3:
                        maple_anim = bn::create_sprite_animate_action_forever(builder.release_build(), 8, maple.tiles_item(), 10, 9, 11, 9);
                        break;
                }
                done = true;
            }
            if (!e_done) {
                switch(e_dir) {
                    case 0:
                        enoki_anim = bn::create_sprite_animate_action_forever(e_builder.release_build(), 8, enoki.tiles_item(), 1, 0, 2, 0);
                        break;
                    case 1:
                        enoki_anim = bn::create_sprite_animate_action_forever(e_builder.release_build(), 8, enoki.tiles_item(), 4, 3, 5, 3);
                        break;
                    case 2:
                        enoki_anim = bn::create_sprite_animate_action_forever(e_builder.release_build(), 8, enoki.tiles_item(), 7, 6, 8, 6);
                        break;
                    case 3:
                        enoki_anim = bn::create_sprite_animate_action_forever(e_builder.release_build(), 8, enoki.tiles_item(), 10, 9, 11, 9);
                        break;
                }
            }
            ...
            int col[8] = {
                local_tileset[close[0] - 1 + (close[2] * width)] > 1,
                local_tileset[close[0] - 1 + (close[3] * width)] > 1,
                local_tileset[close[1] + 1 + (close[2] * width)] > 1,
                local_tileset[close[1] + 1 + (close[3] * width)] > 1,
                local_tileset[close[0] + ((close[2] - 1) * width)] > 1,
                local_tileset[close[0] + ((close[3] - 1) * width)] > 1,
                local_tileset[close[1] + ((close[2] + 1) * width)] > 1,
                local_tileset[close[1] + ((close[3] + 1) * width)] > 1
            };
            //BN_LOG(col[0],col[1],col[2],col[3],col[4],col[5],col[6],col[7]);
            // Key controls
            ...
            // Enoki collision detection
            e_done = true;
            e_is_walking = false;
            x_int = enoki_ptr.x().integer();
            y_int = enoki_ptr.y().integer();
            close[0] = ((x_int + 6 + 31) & (-32)) / 32;
            close[1] = (x_int - 6) / 32;
            close[2] = ((y_int + 31) & (-32)) / 32;
            close[3] = y_int / 32;
            col[0] = local_tileset[close[0] - 1 + (close[2] * width)] > 1;
            col[1] = local_tileset[close[0] - 1 + (close[3] * width)] > 1;
            col[2] = local_tileset[close[1] + 1 + (close[2] * width)] > 1;
            col[3] = local_tileset[close[1] + 1 + (close[3] * width)] > 1;
            col[4] = local_tileset[close[0] + ((close[2] - 1) * width)] > 1;
            col[5] = local_tileset[close[0] + ((close[3] - 1) * width)] > 1;
            col[6] = local_tileset[close[1] + ((close[2] + 1) * width)] > 1;
            col[7] = local_tileset[close[1] + ((close[3] + 1) * width)] > 1;
            ...
            for (int t = 0; t < 16; t++) {
                if (actions[t].tile > 0) {
                    int prolog = (maple_ptr.x().integer() / 32) + ((maple_ptr.y().integer() / 32) * width);
                    BN_LOG(prolog, " - ", actions[t].tile);
                    if (prolog == actions[t].tile) {
                        a_notif.set_visible(true);
                        a_notif.set_x(maple_ptr.x());
                        a_notif.set_y(maple_ptr.y() - 28);
                        if (bn::keypad::a_pressed()) {
                            a_notif.set_visible(false);
                            do_action(actions[t].action);
                        }
                    }

                    //ACTIVATE for determining position of actions
                    //BN_LOG();
                }
            }
